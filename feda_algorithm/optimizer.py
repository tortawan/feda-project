...
class RF_MIMIC:
    ...
    def run(self, random_seed: int = None):
        ...
        self.avg_fitness_history = [] # ADDED: Initialize list for average fitness
        self.timing_info = {'total_time': 0.0, 'iteration_times':[]}
        self.best_solution = None 
        self.best_fitness = -np.inf 
        ...
        if self.population_size == 0: 
            ...
            if self.num_genes == 0:
                ...
                # For 0 pop, 0 gene, avg fitness is same as best
        ...
        current_avg_fitness_for_iteration = -np.inf # Initialize for first iteration
        if self.population.shape[0] > 0: 
            self.fitnesses = self._evaluate_population(self.population)
            if self.fitnesses.size > 0: # ADDED: Check if fitnesses were computed
                 current_avg_fitness_for_iteration = np.mean(self.fitnesses)
        else: 
            self.fitnesses = np.array([])
            if self.num_genes == 0: # For 0-gene problem, avg fitness is the fixed fitness
        ...
        if self.fitnesses is not None and self.fitnesses.size > 0:
            ...
            if self.num_genes == 0 and self.population_size > 0:  
                ...
                current_avg_fitness_for_iteration = self.best_fitness # Avg is same as best for 0-gene

        self.fitness_history.append(self.best_fitness)
        self.avg_fitness_history.append(current_avg_fitness_for_iteration) # ADDED: Store initial average
        ...
        if self.fitnesses is not None and self.fitnesses.size > 0:
            initial_avg_fitness = np.mean(self.fitnesses) # This is current_avg_fitness_for_iteration
            print_debug(f"[RF_MIMIC] Initial Avg Fitness: {initial_avg_fitness:.4f}")
        elif self.num_genes == 0 and self.population_size > 0: # Handle 0-gene case for initial print
        ...
        for iteration in range(self.max_iterations):
            ...
            if self.population_size > 0 and (self.population.shape[0]!= self.population_size or \
                ...
                print_debug(f"FEDA Iter {iteration + 1}: Pop size {self.population.shape} not {self.population_size}x{self.num_genes} after {action_taken}. Adjusting.") 
            ...
            current_iter_avg_fitness = -np.inf # Renamed for clarity within loop
            ...
            if self.population.shape[0] > 0:
                ...
                if self.fitnesses.size > 0:
                    ...
                    current_iter_avg_fitness = np.mean(self.fitnesses) # Calculate current avg
                ...
                # For 0-gene, avg fitness is the same as best (fixed value)
                current_iter_avg_fitness = self.best_fitness 


            self.fitness_history.append(self.best_fitness)
            self.avg_fitness_history.append(current_iter_avg_fitness) # ADDED: Store current average
        ...
        # MODIFIED: Return avg_fitness_history
...
class MIMIC_O2:
    ...
    def run(self, random_seed=None):
        ...
        if self.num_genes == 0:  
            ...
            initial_avg_fitness_val = overall_best_fitness # Avg is same as best for 0-gene
        ...
        self.avg_fitness_history.append(initial_avg_fitness_val) # ADDED: Store initial average
        ...
        if self._current_fitness_scores.size > 0: # Or check initial_avg_fitness_val != -np.inf
        ...
        for iteration in range(self.max_iterations):
            ...
            self.fitness_history.append(overall_best_fitness) 
            self.avg_fitness_history.append(curr_gen_avg_fitness) # ADDED: Store current average
            iter_time_taken = time.time() - iter_start_time
            self.timing_info['iteration_times'].append(iter_time_taken)
